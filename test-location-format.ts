...
async function testLocationFormat() {
  ...
  const { data: orders, error } = await supabase
    .from('orders')
    .select('id, order_number, loading_point_location, unloading_point_location')
    .limit(1);
  ...
  console.log('   JSON:', JSON.stringify(order.loading_point_location, null, 2));
  ...
  console.log('   JSON:', JSON.stringify(order.unloading_point_location, null, 2));
