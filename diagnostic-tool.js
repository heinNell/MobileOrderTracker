...
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://liagltqpeilbswuqcahp.supabase.co';
...
class OrderSystemDiagnostic {
  ...
  async checkUserSetup() {
    try {
      ...
      if (usersWithoutTenant.length > 0) {
        console.log(`⚠️  Users: ${usersWithoutTenant.length} users without tenant assignment`);
        this.issues.push({
          category: 'Users',
        ...
        console.log(`✅ Users: ${users.length} users properly linked to tenants`);
      }
    } catch (error) {
    ...
    try {
      ...
      if (orders.length === 0) {
        ...
        if (orphanedOrders && orphanedOrders.length > 0) {
          console.log(`⚠️  Orders: ${orphanedOrders.length} orphaned orders (no tenant)`);
          this.issues.push({
            category: 'Orders',
  ...
  generateQRSecret() {
    ...
    console.log('QR_CODE_SECRET=' + Buffer.from(Math.random().toString()).toString('base64'));
  }
  ...
  generateReport() {
    ...
    if (this.issues.length === 0) {
      console.log('✅ All checks passed! Your system appears to be working correctly.');
    } else {
    ...
    if (this.fixes.length > 0) {
      ...
      console.log('Run this tool with --fix flag to apply fixes automatically.');
    }
  }
}
